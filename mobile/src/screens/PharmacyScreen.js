...
import { getMedicaments } from '../services/api';
...
export default function PharmacyScreen({ navigation }) {
    ...
    // Données statiques pour les autres onglets
        ...
        try {
            ...
            if (data && data.length > 0) {
                const formattedMedicines = data.map(med => ({
                    id: med.id,
                    name: med.nom,
                    price: parseInt(med.prix),
                    stock: med.stock > 0,
                    image: getImageForCategory(med.categorie),
                    description: med.description || 'Médicament disponible',
                    pharmacie: med.Pharmacien?.nom_pharmacie || 'Pharmacie partenaire'
                }));
    ...
    const getImageForCategory = (categorie) => {
        const images = {
            'Antalgique': '💊',
            'Antibiotique': '💊',
            'AINS': '💊',
    ...
    const handleAddToCart = (item) => {
        if (!item.stock) {
            Alert.alert('Rupture de stock', 'Ce médicament n\'est pas disponible');
            return;
        }
        addToCart(item);
        Alert.alert('Ajouté au panier', `${item.name} a été ajouté`);
    };
        ...
        <View style={styles.itemCard}>
            ...
            >
                ...
                <View style={styles.itemInfo}>
                    ...
                    {item.pharmacie && (
                        <Text style={styles.pharmacyName}>🏪 {item.pharmacie}</Text>
                    )}
                </View>
            </TouchableOpacity>
            <View style={styles.itemActions}>
                ...
                {item.stock && (
                    ...
                    >
                        <Ionicons name="add-circle" size={36} color="#FF3B30" />
                    </TouchableOpacity>
                )}
            </View>
        </View>
    ...
    const renderContent = () => {
        if (selectedTab === 'medicines' && loading) {
            return (
